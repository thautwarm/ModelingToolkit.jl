...
function DiffEqBase.ODEFunction{iip}(sys::AbstractODESystem, dvs = states(sys),
    ...
    f_oop,f_iip = eval_expression ? (mk_function(eval_module, ex) for ex in f_gen) : f_gen
    f(u,p,t) = f_oop(u,p,t)
    ...
    if tgrad
        ...
        tgrad_oop,tgrad_iip = eval_expression ? (mk_function(eval_module, ex) for ex in tgrad_gen) : tgrad_gen
    ...
    if jac
        jac_gen = generate_jacobian(sys, dvs, ps;
                                    ...
                                    expression=Val{eval_expression}, expression_module=eval_module, kwargs...)
        jac_oop,jac_iip = eval_expression ? (mk_function(eval_module, ex) for ex in jac_gen) : jac_gen
